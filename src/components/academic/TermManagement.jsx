...
export default function TermsManager() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      console.error('Error saving term:', error.message);
    }
  };
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2">
          <Card>
            <h2 className="text-xl font-bold mb-4">Terms / Semesters</h2>
            <div className="space-y-3">
              {terms.map(term => (
                <div key={term.id} className="p-3 bg-gray-50 rounded-lg flex justify-between items-center">
                  <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <Button onClick={() => setCurrentTerm(term)} variant="secondary">Edit</Button>
                    <Button onClick={() => handleDelete(term.id)} variant="danger"><Trash2 className="w-4 h-4" /></Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div>
          <Card>
            <h2 className="text-xl font-bold mb-4">{currentTerm ? 'Edit Term' : 'Add New Term'}</h2>
            <form onSubmit={handleSave} className="space-y-4">
              <Input label="Term Name" id="name" name="name" type="text" defaultValue={currentTerm?.name || ''} placeholder="e.g., Fall Semester" required />
              <Input label="Start Date" id="start_date" name="start_date" type="date" defaultValue={currentTerm?.start_date || ''} required />
              <Input label="End Date" id="end_date" name="end_date" type="date" defaultValue={currentTerm?.end_date || ''} required />
              <div className="flex justify-end space-x-3 pt-2">
                {currentTerm && <Button type="button" onClick={() => setCurrentTerm(null)} variant="secondary">Cancel</Button>}
